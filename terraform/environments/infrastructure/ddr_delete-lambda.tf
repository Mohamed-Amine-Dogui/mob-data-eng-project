########################################################################################################################
###  Redshift ddr_delete lambda
########################################################################################################################
module "ddr_delete_lambda" {
  ...
  additional_policy        = data.aws_iam_policy_document.ddr_delete_lambda_policy.json
  attach_additional_policy = true

  lambda_unique_function_name = var.ddr_delete_lambda_unique_function_name
  artifact_bucket_name        = module.lambda_scripts_bucket.s3_bucket
  runtime                     = "python3.9"
  handler                     = var.default_lambda_handler
  main_lambda_file            = "main"
  lambda_base_dir             = "${abspath(path.cwd)}/../../../lambdas/ddr_delete"
  lambda_source_dir           = "${abspath(path.cwd)}/../../../lambdas/ddr_delete/src"
  memory_size                 = 512
  timeout                     = 900
...
########################################################################################################################
### Policy of ddr_delete lambda
########################################################################################################################
data "aws_iam_policy_document" "ddr_delete_lambda_policy" {
  /*
...
########################################################################################################################
resource "aws_kms_grant" "ddr_delete_lambda_grant_kms_key" {
  count             = local.count_in_default
  name              = module.generic_labels.resource["grant"]["id"]
  key_id            = module.rs_mgmt_kms_key.kms_key_id
  grantee_principal = module.ddr_delete_lambda.aws_lambda_function_role_arn
  operations = [
    "Decrypt",
    "Encrypt",
    "GenerateDataKey",
